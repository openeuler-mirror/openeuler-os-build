...
function make_iso_everything_inchroot()
{
    ...
    echo "<h3>Download on windows: <a href=\"http://${RELEASE_SERVER_IP}/${RELEASE_DIR}/${TGZ_NAME}\" target='_blank'>${TGZ_NAME}</a><br />" >> "${html}"
    echo "<p>Download on linux: run \"wget http://${RELEASE_SERVER_IP}/${RELEASE_DIR}/${TGZ_NAME}\"</p>" >> "${html}"
    echo "<p>View the history version, please go to : <a href=\"http://${RELEASE_SERVER_IP}/${PRE_VERSION}\" target='_blank'>http://${RELEASE_SERVER_IP}/${PRE_VERSION}/</a></p></h3></div>" >> "${html}"
    echo "<br />" >> "${html}"
