...
# Returns:
######################
function make_devstation_iso()
{
    chroot_run "cd /home; bash make_version.sh make_devstation_iso_inchroot"
}
...
# Returns:
######################
function make_devstation_iso_inchroot()
{
    get_version
...
repo --name=\"third_repo\" --baseurl=${THIRD_REPO}" /opt/oemaker/config/$ARCH/livecd/devstation_livecd_$ARCH.ks
    ...
    while [ "${num}" -lt 3 ]
    do
        bash -x oemaker -t devstation -p ${PRODUCTS} -v "${OS_VERSION}" -r "" -s "${REPOS}" -u "${STANDARD_PROJECT_REPO}"
        if [ $? -eq 0 ];then
            break
        elif [ $? -eq 133 ]; then
            sleep 60
    ...
    cd /result
    devstation_iso_name=$(ls *.iso)
    if [ -z "${devstation_iso_name}" ]; then
        log_error "No ISO file found in /result"
        exit 1
    fi

    create_checksum "${devstation_iso_name}"
    iso_rpmlist="${devstation_iso_name}.rpmlist"
    rm -rf temp && mkdir temp
    mount "${devstation_iso_name}" temp
    cd temp/Packages
    ls *.rpm > "../../${iso_rpmlist}"
    cd ../..
    umount temp
    ...
    sshcmd "${SSH_CMD}"
    sshscp "${devstation_iso_name} ${devstation_iso_name}.sha256sum ${iso_rpmlist}" "${RELEASE_DIR}"
}
