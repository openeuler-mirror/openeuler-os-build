...
")
	...
	expect <<-END1
		set timeout 300
		spawn scp -o StrictHostKeyChecking=no -o LogLevel=ERROR -o ServerAliveInterval=300 -r ${source_user}@${source_ip}:${source_dir}/* ./tmpdir/
		expect {
			-re "\[P|p]assword:" {
				send "${source_pwd}\r"
			}
	...
	if [ $? -ne 0 ];then
		...
		scp -i ${ssh_key} -o StrictHostKeyChecking=no -o LogLevel=ERROR -o ServerAliveInterval=300 -r tmpdir/* root@${dest_ip}:${latest_iso_dir}/embedded_img/
		if [ $? -ne 0 ];then
